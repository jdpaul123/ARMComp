...
class CPU(MVCListenable):
    ...
    def step(self):
        ...
        reg = self.registers[15].get()
        value = self.memory.get(reg)
        # Decode
        instr = decode(value)
        # Display the CPU state when we have decoded the instruction,
        # before we have executed it
        self.notify_all(CPUStep(self, reg, value, instr))

        if self.condition & instr.cond:
            l_op = self.registers[instr.reg_src1].get()
            r_op = self.registers[instr.reg_src2].get()
            r_op = r_op + instr.offset
            run = self.alu.exec(instr.op, l_op, r_op)
            self.registers[15].value += 1
            self.condition = run[1]
            if instr.op == OpCode.STORE:
                self.memory.put(run[0], self.registers[instr.reg_target].get())
            elif instr.op == OpCode.LOAD:
                val = self.memory.get(run[0])
                self.registers[instr.reg_target].put(val)
            elif instr.op == OpCode.HALT:
                self.halted = True
            else:
                self.registers[instr.reg_target].put(run[0])
        else:
            self.registers[15].value += 1
