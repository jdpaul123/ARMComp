...
class Neg(UnOp):
    ...
    def gen(self, context: Context, target: str) -> str:
        self.left.gen(context, target) # I Don't really get why this is needed or how it workd. I know its a
        #  tree structure but I dont how it workd or the specifics
...
class Seq(Control):
    """exp ; exp. Executes its left and then right operation"""
    ...
    def gen(self, context: Context, target: str):
        """Generate the string("CODE") for the left and right operands"""
        self.left.gen(context, target)
        self.right.gen(context, target)
