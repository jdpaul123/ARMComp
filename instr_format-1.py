...
class Instruction(object):
    ...
    def encode(self) -> int:
        ...
        result = 0
        result = instr_field.insert(self.op.value, result)
...
def decode(word: int) -> Instruction:
        ...
        d_instr_field = OpCode(instr_field.extract(word))
        d_cond_field = CondFlag(cond_field.extract(word))
        d_reg_target_field = reg_target_field.extract(word)
        d_reg_src1_field = reg_src1_field.extract(word)
        d_reg_src2_field = reg_src2_field.extract(word)
        d_offset_field = offset_field.extract_signed(word)

        return Instruction(d_instr_field, d_cond_field, d_reg_target_field,
                           d_reg_src1_field, d_reg_src2_field, d_offset_field)